...
import com.bltu.generator.models.TableAttributeKeyValueTemplateVO;
...
public class ConverterUtil {
	 ...
	 * @param content 装换目标类
	 */
	public static void vueConfigToContent(VueConfig ec, VueContent content) {
		if (!StrUtil.isNullOrEmpty(ec.getTableAlias())) {
			content.setTableAlias(ec.getTableAlias());
		}
		if (!StrUtil.isNullOrEmpty(ec.getPrimaryKey())) {
	 ...
	 *          实体类名
	 */
	public static void routerConfigToContent(ControllerConfig sc, ControllerContent content, String className ,String tableName) {
		if (sc.getTableItem() != null) {
			String loCase = StrUtil.fristToLoCase(className);
			String path = StrUtil.toPath(tableName);
			Map<String, Object> map = new HashMap<>();
			for (TableAttributeKeyValue c : sc.getTableItem()) {
				String key = c.getKey().replace("{C}", className).replace("{c}", loCase).replace("{r}",path);
				String value = c.getValue().replace("{C}", className).replace("{c}", loCase).replace("{r}",path);
				String describe = c.getDescribe().replace("{C}", className).replace("{c}", loCase).replace("{r}",path);
				map.put(c.getKey(), new TableAttributeKeyValue(key, value, describe));
			}
			content.setItem(map);
		}
