...
import com.bltu.generator.entity.TableContent;
import com.bltu.generator.models.DBType;
...
public class DBUtil {
	...
	private static final int DB_CONNECTION_TIMEOUTS_SECONDS = 3;
		...
		if (rs.next()) {
			try {
				...
				try {
					...
					logger.debug("获取表属性一些可能为null的字段失败", e);
				}
			} catch (Exception e) {
				logger.error("获取部分表属性失败:", e);
			}
		}
		return content;
	}
