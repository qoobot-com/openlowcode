...
import com.bltu.generator.options.MapperConfig;
...
public class SetMapperController extends BaseController {
	 ...
	 */
	public void init() {
		logger.debug("初始化SetMapperController...");
		logger.debug("初始化SetMapperController->初始化属性...");
		// 添加右键删除属性
		...
		logger.debug("初始化SetMapperController->初始化模板文件名选择...");
		...
		logger.debug("初始化SetMapperController->初始化配置信息...");
		...
		initLanguage();
		logger.debug("初始化SetMapperController-->成功!");
	}
	...
	public MapperConfig getConfig(String name) {
		...
		return new MapperConfig().initDefaultValue();
	}
	...
	public void onSaveConfig(ActionEvent event) {
		...
		try {
			...
			ConfigUtil.saveSqlAndParamsConfig(getThisConfig(), configName);
