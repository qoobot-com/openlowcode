...
import com.bltu.generator.options.ControllerConfig;
...
public class SetRouterController extends BaseController {
	 ...
	 */
	public void init() {
		logger.debug("初始化SetRouterController...");
		logger.debug("初始化SetRouterController->初始化属性...");
		// 添加右键删除属性
		...
		logger.debug("初始化SetRouterController->初始化模板文件名选择...");
		...
		logger.debug("初始化SetRouterController->初始化配置信息...");
		...
		initLanguage();
		logger.debug("初始化SetRouterController-->成功!");
	}
	...
	public ControllerConfig getConfig(String name) {
		...
		try {
			ControllerConfig config = ConfigUtil.getRouterConfig(name);
		...
		return new ControllerConfig().initDefaultValue();
	}
	 ...
	 * @return
	 */
	public ControllerConfig getThisConfig() {
		logger.debug("执行获取当前页面配置文件...");
		ControllerConfig config = new ControllerConfig(tblPropertyValues, cboTemplate.getValue(), chkOverrideFile.isSelected());
	 ...
	 * @param config
	 */
	public void loadConfig(ControllerConfig config) {
