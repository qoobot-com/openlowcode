...
import com.bltu.generator.options.ServiceConfig;
 ...
 *
 */
public class SetServiceController extends BaseController {
	 ...
	 */
	public void init() {
		logger.debug("初始化SetServiceConterller...");
		logger.debug("初始化SetServiceConterller->初始化属性...");
		// 添加右键删除属性
		...
		logger.debug("初始化SetServiceConterller->初始化模板文件名选择...");
		...
		logger.debug("初始化SetServiceConterller->初始化配置信息...");
		if (indexController.getHistoryConfig() != null) {
			if (indexController.getHistoryConfig().getServiceConfig() == null) {
				loadConfig(getConfig());
			} else {
				loadConfig(indexController.getHistoryConfig().getServiceConfig());
			}
		} else {
		...
		initLanguage();
		logger.debug("初始化SetServiceConterller-->成功!");
	}
	...
	public ServiceConfig getConfig(String name) {
		...
		try {
			ServiceConfig config = ConfigUtil.getServiceConfig(name);
	 ...
	 * @return
	 */
	public ServiceConfig getThisConfig() {
		logger.debug("执行获取当前页面配置文件...");
		ServiceConfig config = new ServiceConfig(tblPropertyValues, cboTemplate.getValue(), chkOverrideFile.isSelected());
	...
	public void onSaveConfig(ActionEvent event) {
		...
		try {
			...
			ConfigUtil.saveServiceConfig(getThisConfig(), configName);
	...
	public void onConfirm(ActionEvent event) {
		indexController.getHistoryConfig().setServiceConfig(getThisConfig());
		getDialogStage().close();
	}
