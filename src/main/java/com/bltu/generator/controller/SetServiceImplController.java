...
import com.bltu.generator.models.TableAttributeKeyValueEditingCell;
import com.bltu.generator.options.ServiceImplConfig;
 ...
 *
 */
public class SetServiceImplController extends BaseController {
	 ...
	 */
	public void init() {
		logger.debug("初始化SetServiceImplController...");
		logger.debug("初始化SetServiceImplController->初始化属性...");
		// 添加右键删除属性
		...
		logger.debug("初始化SetServiceImplController->初始化模板文件名选择...");
		cboTemplate.getItems().addAll(indexController.getTemplateNameItems());
		if (indexController.getTemplateNameItems().contains(Constant.TEMPLATE_NAME_SERVICE_IMPL)) {
			cboTemplate.setValue(Constant.TEMPLATE_NAME_SERVICE_IMPL);
		}
		logger.debug("初始化SetServiceImplController->初始化配置信息...");
		if (indexController.getHistoryConfig() != null) {
			if (indexController.getHistoryConfig().getServiceImplConfig() == null) {
				loadConfig(getConfig());
			} else {
				loadConfig(indexController.getHistoryConfig().getServiceImplConfig());
			}
		} else {
		...
		initLanguage();
		logger.debug("初始化SetServiceImplController-->成功!");
	}
	...
	private void initLanguage() {
		lblTips.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_LBL_TIPS));
		tdDescribe.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_TD_DESCRIBE));
		lblAddCustomProperty.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_LBL_ADD_CUSTOM_PROPERTY));
		lblDescribe.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_LBL_DESCRIBE));
		...
		txtValue.promptTextProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_TXT_VALUE));
		txtDescribe.promptTextProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_TXT_DESCRIBE));
	 ...
	 * @return
	 */
	public ServiceImplConfig getConfig() {
	 ...
	 * @return
	 */
	public ServiceImplConfig getConfig(String name) {
		logger.debug("执行从数据库中获取配置文件...");
		try {
			ServiceImplConfig config = ConfigUtil.getServiceImplConfig(name);
		...
		return new ServiceImplConfig().initDefaultValue();
	}
	 ...
	 * @return
	 */
	public ServiceImplConfig getThisConfig() {
		logger.debug("执行获取当前页面配置文件...");
		ServiceImplConfig config = new ServiceImplConfig(tblPropertyValues, cboTemplate.getValue(), chkOverrideFile.isSelected());
	 ...
	 * @param config
	 */
	public void loadConfig(ServiceImplConfig config) {
	...
	public void onSaveConfig(ActionEvent event) {
		...
		try {
			...
			ConfigUtil.saveServiceImplConfig(getThisConfig(), configName);
	...
	public void onConfirm(ActionEvent event) {
		indexController.getHistoryConfig().setServiceImplConfig(getThisConfig());
		getDialogStage().close();
	}
