/**
 * Copyright (c) Mianlong Wu. 2022-2022. All rights reserved.
 */

package com.bltu.generator.controller;

import com.bltu.generator.AppMain;
import com.bltu.generator.common.*;
import com.bltu.generator.models.TableAttributeEntity;
import com.bltu.generator.models.TableAttributeEntityEditingCell;
import com.bltu.generator.options.DatabaseConfig;
import com.bltu.generator.options.VueConfig;
 ...
 */
public class SetVueAttributeController extends BaseController {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        StringProperty property = AppMain.LANGUAGE.get(LanguageKey.SET_TBL_TIPS);
     ...
     */
    public void init() {
        logger.debug("初始化SetVueAttribute...");
        logger.debug("初始化SetVueAttribute->初始化属性...");
        // 添加右键删除属性
        StringProperty property = AppMain.LANGUAGE.get(LanguageKey.SET_TBL_MENU_ITEM_DELETE);
        String delMenu = property.get() == null ? "删除该属性" : property.get();
        MenuItem item = new MenuItem(delMenu);
        item.setOnAction(event -> {
            TableView.TableViewSelectionModel<TableAttributeEntity> model = tblProperty.selectionModelProperty().get();
            StringProperty delConfirmP = AppMain.LANGUAGE.get(LanguageKey.SET_TBL_MENU_ITEM_DELETE_CONFIRM);
        ...
        tdColumn.setOnEditCommit((TableColumn.CellEditEvent<TableAttributeEntity, String> t) -> {
        ...
        tdSqlType.setOnEditCommit((TableColumn.CellEditEvent<TableAttributeEntity, String> t) -> {
        ...
        tdField.setOnEditCommit((TableColumn.CellEditEvent<TableAttributeEntity, String> t) -> {
        ...
        logger.debug("初始化SetVueAttribute->初始化模板文件名选择...");
        cboTemplate.getItems().addAll(indexController.getTemplateNameItems());
        if (indexController.getTemplateNameItems().contains(Constant.TEMPLATE_NAME_VUE)) {
            cboTemplate.setValue(Constant.TEMPLATE_NAME_VUE);
        }
        logger.debug("初始化SetVueAttribute->初始化配置信息...");
        // 该属性用来判断是否需要重新加载表格数据与主键
        boolean isInitProperyAndKey = true;
        if (indexController.getHistoryConfig() != null) {
            if (indexController.getHistoryConfig().getVueConfig() == null) {
                loadConfig(getConfig());
            } else {
                logger.debug("加载数据...从上一次保存获取");
                loadConfig(indexController.getHistoryConfig().getVueConfig());
        ...
        logger.debug("初始化SetVueAttribute-->成功!");
    }
    ...
    private void initLanguage() {
        tdIsCreate.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_ENTITY_TD_CREATE));
        tdColumn.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_ENTITY_TD_COLUMN));
        tdSqlType.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_ENTITY_TD_SQL_TYPE));
        tdJavaType.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_ENTITY_TD_JAVA_TYPE));
        tdField.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_ENTITY_TD_FIELD));
        lblAddCustomProperty.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_LBL_ADD_CUSTOM_PROPERTY));
        lblKey.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_ENTITY_LBL_KEY));
        lblValue.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_ENTITY_LBL_VALUE));
        btnSaveConfig.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_BTN_SAVE_CONFIG));
        btnAddProperty.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_BTN_ADD_PROPERTY));
        btnConfirm.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_BTN_CONFIRM));
        btnCancel.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_BTN_CANCEL));
        txtKey.promptTextProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_TXT_KEY));
        txtKey.promptTextProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_ENTITY_TXT_KEY));
        txtValue.promptTextProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_ENTITY_TXT_VALUE));
        chkOverrideFile.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_CHK_OVERRIDE_FILE));
        lblTemplate.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_LBL_TEMPLATE));
        cboTemplate.promptTextProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_CBO_TEMPLATE));
        lblTableAlias.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_ENTITY_LBL_TABLE_ALIAS));
        txtTableAlias.promptTextProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_ENTITY_TXT_TABLE_ALIAS));
        lblPrimaryKey.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_ENTITY_LBL_PRIMARY_KEY));
        txtPrimaryKey.promptTextProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_ENTITY_TXT_PRIMARY_KEY));
        chkFieldCamel.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SET_CHK_FIELD_CAMEL));
    }
    /**
     * 初始化表格属性
     */
    public void initTablePropery(DatabaseConfig selectedDatabaseConfig, String selectedTableName) {
        try {
            ...
            logger.debug("执行获取表的列数据-->成功!");
        } catch (Exception e) {
            logger.error("执行获取表的列数据-->失败:", e);
     ...
     * @return
     */
    public VueConfig getConfig(String name) {
        logger.debug("执行从数据库中获取配置文件...");
        try {
            VueConfig config = ConfigUtil.getVueConfig(name);
            logger.debug("执行获取配置文件-->成功!");
            if (config != null) {
                return config;
            }
        } catch (Exception e) {
            logger.error("执行从数据库中获取配置文件-->失败:", e);
            AlertUtil.showErrorAlert("执行获得配置文件-->失败:" + e);
        }
        return new VueConfig();
    }
     ...
     * @return
     */
    public VueConfig getThisConfig() {
        logger.debug("执行获取当前页面配置文件...");
     ...
     * @return
     */
    public VueConfig getThisConfigHasProperty() {
        logger.debug("执行获取当前页面带有表格数据配置文件...");
        VueConfig config = new VueConfig();
        config.setFieldCamel(chkFieldCamel.isSelected());
        config.setOverrideFile(chkOverrideFile.isSelected());
        config.setTemplateName(cboTemplate.getValue());
        ...
        logger.debug("执行获取当前页面带有表格数据配置文件-->成功!");
        return config;
    }
     ...
     * @param config
     */
    public void loadConfig(VueConfig config) {
        logger.debug("执行加载配置文件到当前页面...");
    ...
    public void onSaveConfig(ActionEvent event) {
        logger.debug("执行将配置文件保存到数据库...");
        try {
            String configName = indexController.getHistoryConfigName();
            if (StrUtil.isNullOrEmpty(configName)) {
                configName = Constant.DEFAULT;
            }
            ConfigUtil.saveVueConfig(getThisConfig(), configName);
            logger.debug("执行将配置文件保存到数据库-->成功!");
            AlertUtil.showInfoAlert("保存配置信息成功!");
        } catch (Exception e) {
            logger.error("执行将配置文件保存到数据库-->失败:", e);
            AlertUtil.showErrorAlert("执行获得配置文件-->失败:" + e);
        }
    }
    ...
    public void onCancel(ActionEvent event) {
        StringProperty property = AppMain.LANGUAGE.get(LanguageKey.SET_BTN_CANCEL_TIPS);
        String tips = property == null ? "如果取消,全部的设置都将恢复到默认值,确定取消吗?" : property.get();
        boolean result = AlertUtil.showConfirmAlert(tips);
        if (result) {
            indexController.getHistoryConfig().setVueConfig(null);
    ...
    public void onConfirm(ActionEvent event) {
        indexController.getHistoryConfig().setVueConfig(getThisConfigHasProperty());
        getDialogStage().close();
    }
