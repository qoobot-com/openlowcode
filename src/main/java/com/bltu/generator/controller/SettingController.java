...
public class SettingController extends BaseController {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		lblLanguage.textProperty().bind(AppMain.LANGUAGE.get(LanguageKey.SETTING_LBL_LANGUAGE));
		...
		lblEnglish.setOnMouseClicked(ev -> {
			AppMain.loadLanguage(Locale.ENGLISH);
			closeDialogStage();
		});
		lblSimpleChinese.setOnMouseClicked(ev -> {
			AppMain.loadLanguage(Locale.SIMPLIFIED_CHINESE);
			closeDialogStage();
		});
	}
