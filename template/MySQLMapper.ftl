...
<mapper namespace="${content.dao.classPackage}.${content.dao.className}">
	...
	<select id="${content.dao.item.select.value}" parameterType="${content.assist.classPackage}.Assist" resultMap="${content.mapper.item.resultMap.value}">
		...
		<choose>
			<when test="startRow !=null and rowSize !=null">LIMIT ${r'#{startRow}'},${r'#{rowSize}'} </when>
			<when test="rowSize !=null">LIMIT ${r'#{rowSize}'}</when>
		</choose>
	</select>
	...
	<select id="${content.dao.item.selectByObjSingle.value}" parameterType="${content.entity.classPackage}.${content.entity.className}" resultMap="${content.mapper.item.resultMap.value}">
		...
		from ${content.entity.tableName}
		<where>
		<#if content.entity.otherAttrs??>
		<#list content.entity.otherAttrs as item>
		<#if item.javaType  ==  "java.util.Date">
		<if test="${item.field} != null"> and ${item.columnName} = ${r'#{'}${item.field}${r'}'}</if>
        <if test="${item.field}Start != null"> and ${item.columnName} &gt;= ${r'#{'}${item.field}Start${r'}'}</if>
        <if test="${item.field}End != null"> and ${item.columnName} &lt;= ${r'#{'}${item.field}End${r'}'}</if>
        <#else>
        <if test="${item.field} != null"> and ${item.columnName} = ${r'#{'}${item.field}${r'}'}</if>
        </#if>
		</#list>
		</#if>
		<#if content.entity.cantNullAttrs??>
		<#list content.entity.cantNullAttrs as item>
		<#if item.javaType  ==  "java.util.Date">
		<if test="${item.field} != null"> and ${item.columnName} = ${r'#{'}${item.field}${r'}'}</if>
		<if test="${item.field}Start != null"> and ${item.columnName} &gt;= ${r'#{'}${item.field}Start${r'}'}</if>
		<if test="${item.field}End != null"> and ${item.columnName} &lt;= ${r'#{'}${item.field}End${r'}'}</if>
		...
		<#if content.entity.primaryKeyAttr??>
		<if test="${content.entity.primaryKeyAttr.field} != null">  and ${content.entity.primaryKeyAttr.columnName} = ${r'#{'}${content.entity.primaryKeyAttr.field}${r'}'}</if>
		 </#if>
		</where>
		LIMIT 1
	</select>
    ...
    <!-- 通过${content.entity.className}的id将数据库表中对应的数据删除 -->
	<delete id="${content.dao.item.deleteByIds.value}" parameterType="${content.entity.primaryKeyAttr.javaType}">
		delete from ${content.entity.tableName}
		where  ${content.entity.primaryKeyAttr.columnName}  in
